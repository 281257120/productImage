 ...
 * @LastEditTime: 2024-05-15 09:47:32
 * @LastEditors: Liu Yang
 * @Description:
 * @FilePath: \drag-demo\src\layouts\MainLayout.vue
-->
<template>
  <q-layout view="hHh Lpr fFf" class="main-layout" :class="{ 'gray-mode': grayActive }">
    <q-drawer v-if="asideMenus?.length && globalLayout.header" v-model="drawer" show-if-above :width="180"
      :breakpoint="500" bordered :mini="globalLayout.leftDrawerMini">
      <q-list class="drawer-menu-list">
        <template v-for="(menuItem, index) in asideMenus" :key="index">
          <q-item v-ripple clickable active-class="menu-active-item" :active="menuItem.name === currentRouteName"
            ...
            <q-tooltip v-if="globalLayout.leftDrawerMini" transition-show="scale" transition-hide="scale"
    ...
    <q-page-container class="no-padding">
      <q-page class="interaction-root">
        <!-- vc-viewer -->
        <component :is="mode === '3d' ? MainViewer : MainMap">
          <!-- header -->

          <header v-if="globalLayout.header" elevated class="main-header-container text-h4 text-center ">
            <main-header />
          </header>
...
import MainMap from '@src/components/map/Index.vue'
...
.main-layout {
  ...
  .main-header-container {
    ...
    background-image: v-bind('theme.header.themeHeaderBg');
